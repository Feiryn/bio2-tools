...
// bi2a 804c firmware
const BI2A_804C_FIRMWARE_START_BYTES: [u8; 6] = [0xBF, 0x72, 0x8E, 0x86, 0x64, 0x6A];
const BI2A_804C_END_BYTES: [u8; 6] = [0xF6, 0xC7, 0x20, 0x0F, 0x78, 0xFF];

// bi2x firmware
const BI2X_FIRMWARE_START_BYTES: [u8; 6] = [0xFF, 0x0D, 0x7F, 0xFC, 0xFE, 0x7F];
const BI2X_FIRMWARE_END_BYTES: [u8; 5] = [0xC7, 0x20, 0x1F, 0x70, 0xFF];
...
fn main() {
    ...
    let bi2a_804c_firmware = find_firmware(
        &file_content,
        &BI2A_804C_FIRMWARE_START_BYTES,
        &BI2A_804C_END_BYTES,
    );
    ...
    let bi2a_804c_firmware = bi2a_804c_firmware.expect("bi2a 804c firmware byte sequence not found in the file. The file may be too recent for the tool. Please report this issue to the developer.");
    ...
    bio2wrfirm_buffer[bio2wrfirm.len()..(bio2wrfirm.len() + bi2x_firmware.len())]
        .copy_from_slice(&bi2x_firmware); // Append the encrypted kernel at the end
    let crc = compute_crc(&bio2wrfirm_buffer[2..]);
    bio2wrfirm_buffer[0] = crc[0];
    bio2wrfirm_buffer[1] = crc[1];

    // Decrypt bi2x firmware
    let bi2a_804c_firmware = crypt::decrypt_aio_iob_firmware(&bi2a_804c_firmware);
    ...
    std::fs::write(
        args.output_dir.join("bi2a_804c_firmware.bin"),
        bi2a_804c_firmware,
    )
